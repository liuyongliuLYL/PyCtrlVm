...
class Vcenter():
    ...
    def clone_vm(self,
        ...
        # 数据中心
        datacenter = self.get_obj(vimtype = [vim.Datacenter], name = datacenter_name)
        # 文件夹位置
        if vm_folder:
            destfolder = self.get_obj(vimtype = [vim.Folder], name = vm_folder)
        else:
        ...
        if datastore_name:
            ...
            datastore = self.get_obj(
                vimtype = [vim.Datastore], name = template.datastore[0].info.name)

        # 集群
        cluster = self.get_obj(vimtype = [vim.ClusterComputeResource], name = cluster_name)
        # 资源池
        if resource_pool:
            resource_pool = self.get_obj(vimtype = [vim.ResourcePool], name = resource_pool)
        else:
